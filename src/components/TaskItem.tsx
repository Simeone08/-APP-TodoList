...
export function TaskItem({ task, editTask, removeTask } : TaskItemProps){
    const [isEditing, setIsEditing] = useState(false)
    const [ taskNewTitleValue, setTaskNewTitleValue ] = useState(task.title)
    const textInpuRef = useRef<TextInput>(null)
    ...
    useEffect(() => {
        if (textInpuRef.current) {
          if (isEditing) {
            textInpuRef.current.focus();
          } else {
            textInpuRef.current.blur();
          }
        }
      }, [isEditing])
...
function toggleTaskDone(id: any): void {
